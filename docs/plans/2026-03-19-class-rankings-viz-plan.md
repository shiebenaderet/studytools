# Class Rankings Visualization Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a line chart and animated bar chart race showing class average scores over time on the teacher dashboard.

**Architecture:** New `rankings-viz.js` file containing a `RankingsViz` object that handles all canvas rendering, animation, and controls. Dashboard calls it from `loadOverview()` with data and class info. A new container div is added to the HTML between the class leaderboard and existing replay panel.

**Tech Stack:** Vanilla JS, HTML5 Canvas, CSS, Supabase (read-only queries).

**Spec:** `docs/plans/2026-03-19-class-rankings-viz-design.md`

---

### Task 1: HTML Container and Script Tag

**Files:**
- Modify: `study-tools/dashboard/index.html:105-107`

- [ ] **Step 1: Add the rankings viz container between class leaderboard and replay**

In `index.html`, find lines 105-107:
```html
            <div id="overview-class-lb"></div>
            <div id="overview-replay"></div>
            <div id="overview-leaderboard"></div>
```

Insert a new container between `overview-class-lb` and `overview-replay`:
```html
            <div id="overview-class-lb"></div>
            <div id="overview-rankings-viz"></div>
            <div id="overview-replay"></div>
            <div id="overview-leaderboard"></div>
```

- [ ] **Step 2: Add the script tag for rankings-viz.js**

Find line 152:
```html
    <script src="dashboard.js"></script>
```

Add the new script BEFORE `dashboard.js`:
```html
    <script src="rankings-viz.js"></script>
    <script src="dashboard.js"></script>
```

- [ ] **Step 3: Commit**

```bash
git add study-tools/dashboard/index.html
git commit -m "feat: add rankings viz container and script tag to dashboard"
```

---

### Task 2: RankingsViz Skeleton — Data Processing

**Files:**
- Create: `study-tools/dashboard/rankings-viz.js`

- [ ] **Step 1: Create the file with data processing logic**

Create `study-tools/dashboard/rankings-viz.js` with the data processing foundation. The `render()` method is the entry point called by Dashboard. It processes data, renders a line chart, and renders a bar race. The `_icon()` helper creates Font Awesome icon elements without using innerHTML.

```js
// Class Rankings Visualization — line chart + bar chart race
var RankingsViz = {

    COLORS: ['#4e79a7', '#f28e2b', '#e15759', '#76b7b2', '#59a14f', '#edc948', '#b07aa1', '#9c755f'],

    _icon(className) {
        var i = document.createElement('i');
        i.className = className;
        return i;
    },

    // Process raw snapshot rows into per-class, per-date averages
    // Input: array of { student_id, score, class_id, snapshot_date }
    // Output: { dates: ['2026-03-15', ...], classes: { classId: { name, color, averages: [avg, ...] } } }
    processData(snapshots, classNameMap) {
        // Collect unique dates sorted
        var dateSet = {};
        for (var i = 0; i < snapshots.length; i++) {
            dateSet[snapshots[i].snapshot_date] = true;
        }
        var dates = Object.keys(dateSet).sort();

        // Group by class_id + date
        var classDateTotals = {}; // { classId: { date: { total, count } } }
        for (var i = 0; i < snapshots.length; i++) {
            var s = snapshots[i];
            if (!s.class_id) continue;
            if (!classDateTotals[s.class_id]) classDateTotals[s.class_id] = {};
            if (!classDateTotals[s.class_id][s.snapshot_date]) {
                classDateTotals[s.class_id][s.snapshot_date] = { total: 0, count: 0 };
            }
            classDateTotals[s.class_id][s.snapshot_date].total += s.score;
            classDateTotals[s.class_id][s.snapshot_date].count++;
        }

        // Build output
        var classIds = Object.keys(classDateTotals);
        var classes = {};
        for (var c = 0; c < classIds.length; c++) {
            var cid = classIds[c];
            var averages = [];
            for (var d = 0; d < dates.length; d++) {
                var entry = classDateTotals[cid][dates[d]];
                averages.push(entry ? Math.round(entry.total / entry.count) : null);
            }
            classes[cid] = {
                name: classNameMap[cid] || 'Unknown',
                color: this.COLORS[c % this.COLORS.length],
                averages: averages
            };
        }

        return { dates: dates, classes: classes };
    },

    // Main render entry point — called from Dashboard
    render(container, snapshots, classNameMap) {
        container.textContent = '';

        if (!snapshots || snapshots.length === 0) return;

        var data = this.processData(snapshots, classNameMap);
        var classIds = Object.keys(data.classes);

        if (classIds.length === 0) {
            var msg = document.createElement('p');
            msg.style.cssText = 'color:var(--text-muted);text-align:center;padding:24px;';
            msg.textContent = 'No class data available for this unit.';
            container.appendChild(msg);
            return;
        }

        // Section heading
        var heading = document.createElement('h3');
        heading.style.cssText = 'color:var(--text);margin-bottom:16px;';
        heading.appendChild(this._icon('fas fa-chart-line'));
        heading.appendChild(document.createTextNode(' Class Rankings Over Time'));
        container.appendChild(heading);

        // Line chart
        var lineContainer = document.createElement('div');
        lineContainer.className = 'rankings-line-chart';
        container.appendChild(lineContainer);
        this._renderLineChart(lineContainer, data);

        // Bar chart race
        var raceContainer = document.createElement('div');
        raceContainer.className = 'rankings-bar-race';
        container.appendChild(raceContainer);
        this._renderBarRace(raceContainer, data);
    },

    // Placeholder — implemented in Task 3
    _renderLineChart(container, data) {},

    // Placeholder — implemented in Task 5
    _renderBarRace(container, data) {},
};
```

- [ ] **Step 2: Commit**

```bash
git add study-tools/dashboard/rankings-viz.js
git commit -m "feat: add RankingsViz skeleton with data processing"
```

---

### Task 3: Line Chart Rendering

**Files:**
- Modify: `study-tools/dashboard/rankings-viz.js`

- [ ] **Step 1: Replace the `_renderLineChart` placeholder**

Replace the placeholder `_renderLineChart` method with the full implementation:

```js
    _renderLineChart(container, data) {
        var classIds = Object.keys(data.classes);
        var dates = data.dates;
        if (dates.length === 0) return;

        // Canvas setup with HiDPI
        var canvas = document.createElement('canvas');
        var dpr = window.devicePixelRatio || 1;
        var width = container.clientWidth || 600;
        var height = 250;
        canvas.width = width * dpr;
        canvas.height = height * dpr;
        canvas.style.width = width + 'px';
        canvas.style.height = height + 'px';
        var ctx = canvas.getContext('2d');
        ctx.scale(dpr, dpr);

        // Layout constants
        var pad = { top: 20, right: 20, bottom: 40, left: 50 };
        var chartW = width - pad.left - pad.right;
        var chartH = height - pad.top - pad.bottom;

        // Find y range
        var yMax = 0;
        for (var c = 0; c < classIds.length; c++) {
            var avgs = data.classes[classIds[c]].averages;
            for (var d = 0; d < avgs.length; d++) {
                if (avgs[d] !== null && avgs[d] > yMax) yMax = avgs[d];
            }
        }
        yMax = Math.ceil(yMax / 10) * 10 || 10; // round up to nearest 10

        // Draw axes
        ctx.strokeStyle = '#ddd';
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(pad.left, pad.top);
        ctx.lineTo(pad.left, pad.top + chartH);
        ctx.lineTo(pad.left + chartW, pad.top + chartH);
        ctx.stroke();

        // Y-axis labels
        ctx.fillStyle = '#888';
        ctx.font = '11px system-ui, sans-serif';
        ctx.textAlign = 'right';
        var yTicks = 5;
        for (var i = 0; i <= yTicks; i++) {
            var val = Math.round(yMax * i / yTicks);
            var y = pad.top + chartH - (chartH * i / yTicks);
            ctx.fillText(val, pad.left - 8, y + 4);
            if (i > 0) {
                ctx.strokeStyle = '#eee';
                ctx.beginPath();
                ctx.moveTo(pad.left, y);
                ctx.lineTo(pad.left + chartW, y);
                ctx.stroke();
            }
        }

        // X-axis labels
        ctx.textAlign = 'center';
        ctx.fillStyle = '#888';
        var maxLabels = Math.min(dates.length, 10);
        var labelStep = Math.max(1, Math.floor(dates.length / maxLabels));
        for (var d = 0; d < dates.length; d += labelStep) {
            var x = pad.left + (chartW * d / Math.max(dates.length - 1, 1));
            var parts = dates[d].split('-');
            var label = parseInt(parts[1]) + '/' + parseInt(parts[2]);
            ctx.fillText(label, x, pad.top + chartH + 20);
        }

        // Store data points for hit testing
        var dataPoints = [];

        // Draw lines
        for (var c = 0; c < classIds.length; c++) {
            var cls = data.classes[classIds[c]];
            ctx.strokeStyle = cls.color;
            ctx.lineWidth = 2.5;
            ctx.beginPath();
            var started = false;
            for (var d = 0; d < dates.length; d++) {
                if (cls.averages[d] === null) { started = false; continue; }
                var x = pad.left + (chartW * d / Math.max(dates.length - 1, 1));
                var y = pad.top + chartH - (chartH * cls.averages[d] / yMax);
                if (!started) { ctx.moveTo(x, y); started = true; }
                else ctx.lineTo(x, y);

                dataPoints.push({ x: x, y: y, classId: classIds[c], date: dates[d], avg: cls.averages[d] });
            }
            ctx.stroke();

            // Draw dots
            ctx.fillStyle = cls.color;
            for (var d = 0; d < dates.length; d++) {
                if (cls.averages[d] === null) continue;
                var x = pad.left + (chartW * d / Math.max(dates.length - 1, 1));
                var y = pad.top + chartH - (chartH * cls.averages[d] / yMax);
                ctx.beginPath();
                ctx.arc(x, y, 3.5, 0, Math.PI * 2);
                ctx.fill();
            }
        }

        container.appendChild(canvas);

        // Tooltip (DOM element)
        var tooltip = document.createElement('div');
        tooltip.className = 'rankings-tooltip';
        tooltip.style.display = 'none';
        container.appendChild(tooltip);
        container.style.position = 'relative';

        canvas.addEventListener('mousemove', function(e) {
            var rect = canvas.getBoundingClientRect();
            var mx = e.clientX - rect.left;
            var my = e.clientY - rect.top;
            var closest = null;
            var closestDist = 20; // max hover distance in px
            for (var i = 0; i < dataPoints.length; i++) {
                var dx = dataPoints[i].x - mx;
                var dy = dataPoints[i].y - my;
                var dist = Math.sqrt(dx * dx + dy * dy);
                if (dist < closestDist) {
                    closestDist = dist;
                    closest = dataPoints[i];
                }
            }
            if (closest) {
                var cls = data.classes[closest.classId];
                var parts = closest.date.split('-');
                var dateStr = parseInt(parts[1]) + '/' + parseInt(parts[2]);
                tooltip.textContent = '';
                var nameSpan = document.createElement('strong');
                nameSpan.style.color = cls.color;
                nameSpan.textContent = cls.name;
                tooltip.appendChild(nameSpan);
                tooltip.appendChild(document.createElement('br'));
                tooltip.appendChild(document.createTextNode(dateStr + ': ' + closest.avg + ' avg'));
                tooltip.style.display = 'block';
                tooltip.style.left = Math.min(closest.x + 12, width - 120) + 'px';
                tooltip.style.top = (closest.y - 10) + 'px';
            } else {
                tooltip.style.display = 'none';
            }
        });

        canvas.addEventListener('mouseleave', function() {
            tooltip.style.display = 'none';
        });

        // Legend
        var legend = document.createElement('div');
        legend.className = 'rankings-legend';
        for (var c = 0; c < classIds.length; c++) {
            var cls = data.classes[classIds[c]];
            var item = document.createElement('span');
            item.className = 'rankings-legend-item';
            var swatch = document.createElement('span');
            swatch.className = 'rankings-legend-swatch';
            swatch.style.backgroundColor = cls.color;
            item.appendChild(swatch);
            item.appendChild(document.createTextNode(cls.name));
            legend.appendChild(item);
        }
        container.appendChild(legend);

        // Handle resize
        var self = this;
        var resizeTimer;
        window.addEventListener('resize', function() {
            clearTimeout(resizeTimer);
            resizeTimer = setTimeout(function() {
                self._renderLineChart(container, data);
            }, 200);
        });

        // 1 day note
        if (dates.length === 1) {
            var note = document.createElement('p');
            note.className = 'rankings-note';
            note.textContent = 'More days of data needed to show trends.';
            container.appendChild(note);
        }
    },
```

- [ ] **Step 2: Commit**

```bash
git add study-tools/dashboard/rankings-viz.js
git commit -m "feat: implement line chart for class averages over time"
```

---

### Task 4: Line Chart CSS

**Files:**
- Modify: `study-tools/dashboard/dashboard.css`

- [ ] **Step 1: Add styles at the end of the file**

Append to the end of `dashboard.css`:

```css
/* Rankings Visualization */
.rankings-line-chart {
    position: relative;
    margin-bottom: 24px;
}

.rankings-line-chart canvas {
    display: block;
    width: 100%;
}

.rankings-tooltip {
    position: absolute;
    background: var(--card-bg, #fff);
    border: 1px solid var(--border, #e0e0e0);
    border-radius: 6px;
    padding: 6px 10px;
    font-size: 0.8rem;
    pointer-events: none;
    z-index: 10;
    box-shadow: 0 2px 8px rgba(0,0,0,0.12);
    white-space: nowrap;
}

.rankings-legend {
    display: flex;
    flex-wrap: wrap;
    gap: 12px;
    justify-content: center;
    margin-top: 8px;
}

.rankings-legend-item {
    display: flex;
    align-items: center;
    gap: 4px;
    font-size: 0.85rem;
    color: var(--text, #333);
}

.rankings-legend-swatch {
    width: 12px;
    height: 12px;
    border-radius: 2px;
    display: inline-block;
}

.rankings-note {
    text-align: center;
    color: var(--text-muted, #999);
    font-size: 0.85rem;
    margin-top: 8px;
}
```

- [ ] **Step 2: Commit**

```bash
git add study-tools/dashboard/dashboard.css
git commit -m "feat: add line chart styles to dashboard"
```

---

### Task 5: Bar Chart Race — Rendering and Animation

**Files:**
- Modify: `study-tools/dashboard/rankings-viz.js`

- [ ] **Step 1: Replace the `_renderBarRace` placeholder**

Replace the placeholder `_renderBarRace` method with the full implementation. Note: all icon elements are created using `this._icon()` (DOM method), not innerHTML. The play button text is updated by clearing `textContent` and appending a new icon element.

```js
    _renderBarRace(container, data) {
        var classIds = Object.keys(data.classes);
        var dates = data.dates;
        if (dates.length === 0 || classIds.length === 0) return;

        var self = this;

        // Sub-heading
        var heading = document.createElement('h4');
        heading.style.cssText = 'color:var(--text);margin-bottom:12px;';
        heading.appendChild(self._icon('fas fa-play-circle'));
        heading.appendChild(document.createTextNode(' Class Race'));
        container.appendChild(heading);

        // Date display
        var dateDisplay = document.createElement('div');
        dateDisplay.className = 'race-date-display';
        container.appendChild(dateDisplay);

        // Canvas
        var canvas = document.createElement('canvas');
        var dpr = window.devicePixelRatio || 1;
        var width = container.clientWidth || 600;
        var barHeight = 36;
        var barGap = 8;
        var height = classIds.length * (barHeight + barGap) + 20;
        canvas.width = width * dpr;
        canvas.height = height * dpr;
        canvas.style.width = width + 'px';
        canvas.style.height = height + 'px';
        var ctx = canvas.getContext('2d');
        ctx.scale(dpr, dpr);
        container.appendChild(canvas);

        // Controls
        var controls = document.createElement('div');
        controls.className = 'race-controls';

        var playBtn = document.createElement('button');
        playBtn.className = 'btn race-play-btn';
        playBtn.appendChild(self._icon('fas fa-play'));
        controls.appendChild(playBtn);

        function setPlayIcon(iconClass) {
            playBtn.textContent = '';
            playBtn.appendChild(self._icon(iconClass));
        }

        var speedWrap = document.createElement('div');
        speedWrap.className = 'race-speed-wrap';
        var speedLabel = document.createElement('label');
        speedLabel.textContent = 'Speed ';
        var speedSlider = document.createElement('input');
        speedSlider.type = 'range';
        speedSlider.min = '0.5';
        speedSlider.max = '3';
        speedSlider.step = '0.5';
        speedSlider.value = '1';
        speedSlider.className = 'race-speed-slider';
        var speedReadout = document.createElement('span');
        speedReadout.className = 'race-speed-readout';
        speedReadout.textContent = '1x';
        speedSlider.addEventListener('input', function() {
            speedReadout.textContent = speedSlider.value + 'x';
        });
        speedWrap.appendChild(speedLabel);
        speedWrap.appendChild(speedSlider);
        speedWrap.appendChild(speedReadout);
        controls.appendChild(speedWrap);

        var progress = document.createElement('span');
        progress.className = 'race-progress';
        controls.appendChild(progress);

        container.appendChild(controls);

        if (dates.length <= 1) {
            playBtn.disabled = true;
            progress.textContent = 'Day 1 of 1';
        }

        // Animation state
        var currentDay = 0;
        var playing = false;
        var animFrame = null;
        var transitioning = false;

        // Build snapshots per day: [{ classId, name, color, avg }]
        var daySnapshots = [];
        for (var d = 0; d < dates.length; d++) {
            var snap = [];
            for (var c = 0; c < classIds.length; c++) {
                var cls = data.classes[classIds[c]];
                var avg = cls.averages[d];
                if (avg !== null) {
                    snap.push({ classId: classIds[c], name: cls.name, color: cls.color, avg: avg });
                }
            }
            snap.sort(function(a, b) { return b.avg - a.avg; });
            daySnapshots.push(snap);
        }

        // Find global max for consistent bar scaling
        var maxAvg = 1;
        for (var d = 0; d < daySnapshots.length; d++) {
            for (var i = 0; i < daySnapshots[d].length; i++) {
                if (daySnapshots[d][i].avg > maxAvg) maxAvg = daySnapshots[d][i].avg;
            }
        }

        // Current animated positions: { classId: { y, barW, avg, name, color } }
        var positions = {};
        var labelWidth = 100;
        var barAreaW = width - labelWidth - 60;

        function initPositions(dayIdx) {
            var snap = daySnapshots[dayIdx] || [];
            positions = {};
            for (var i = 0; i < snap.length; i++) {
                positions[snap[i].classId] = {
                    y: i * (barHeight + barGap),
                    barW: (snap[i].avg / maxAvg) * barAreaW,
                    avg: snap[i].avg,
                    name: snap[i].name,
                    color: snap[i].color
                };
            }
        }

        function formatDate(dateStr) {
            var d = new Date(dateStr + 'T12:00:00');
            var today = new Date().toISOString().slice(0, 10);
            var yesterday = new Date(Date.now() - 86400000).toISOString().slice(0, 10);
            if (dateStr === today) return 'Today';
            if (dateStr === yesterday) return 'Yesterday';
            return d.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
        }

        function drawFrame() {
            ctx.clearRect(0, 0, width, height);

            var classIdsDrawn = Object.keys(positions);
            for (var i = 0; i < classIdsDrawn.length; i++) {
                var p = positions[classIdsDrawn[i]];

                // Bar with rounded corners
                ctx.fillStyle = p.color;
                ctx.beginPath();
                var radius = 4;
                var bx = labelWidth;
                var by = p.y + 2;
                var bw = Math.max(p.barW, 2);
                var bh = barHeight - 4;
                ctx.moveTo(bx + radius, by);
                ctx.lineTo(bx + bw - radius, by);
                ctx.quadraticCurveTo(bx + bw, by, bx + bw, by + radius);
                ctx.lineTo(bx + bw, by + bh - radius);
                ctx.quadraticCurveTo(bx + bw, by + bh, bx + bw - radius, by + bh);
                ctx.lineTo(bx + radius, by + bh);
                ctx.quadraticCurveTo(bx, by + bh, bx, by + bh - radius);
                ctx.lineTo(bx, by + radius);
                ctx.quadraticCurveTo(bx, by, bx + radius, by);
                ctx.fill();

                // Label
                ctx.fillStyle = '#333';
                ctx.font = '13px system-ui, sans-serif';
                ctx.textAlign = 'right';
                ctx.fillText(p.name, labelWidth - 8, p.y + barHeight / 2 + 5);

                // Score
                ctx.fillStyle = '#666';
                ctx.textAlign = 'left';
                ctx.fillText(Math.round(p.avg), labelWidth + bw + 8, p.y + barHeight / 2 + 5);
            }
        }

        function updateProgress() {
            progress.textContent = 'Day ' + (currentDay + 1) + ' of ' + dates.length;
            dateDisplay.textContent = formatDate(dates[currentDay]);
        }

        // Easing
        function easeInOut(t) {
            return t < 0.5 ? 2 * t * t : 1 - Math.pow(-2 * t + 2, 2) / 2;
        }

        function transitionToDay(targetDay, callback) {
            var fromPos = {};
            for (var k in positions) fromPos[k] = { y: positions[k].y, barW: positions[k].barW, avg: positions[k].avg };

            // Target positions
            var toSnap = daySnapshots[targetDay] || [];
            var targetPos = {};
            for (var i = 0; i < toSnap.length; i++) {
                targetPos[toSnap[i].classId] = {
                    y: i * (barHeight + barGap),
                    barW: (toSnap[i].avg / maxAvg) * barAreaW,
                    avg: toSnap[i].avg,
                    name: toSnap[i].name,
                    color: toSnap[i].color
                };
            }

            // New classes that weren't in previous day — start from 0
            for (var cid in targetPos) {
                if (!fromPos[cid]) {
                    fromPos[cid] = { y: targetPos[cid].y, barW: 0, avg: 0 };
                    positions[cid] = {
                        y: targetPos[cid].y, barW: 0, avg: 0,
                        name: targetPos[cid].name, color: targetPos[cid].color
                    };
                }
            }

            var duration = 2000 / parseFloat(speedSlider.value);
            var start = performance.now();
            transitioning = true;

            function step(now) {
                var elapsed = now - start;
                var t = Math.min(elapsed / duration, 1);
                var et = easeInOut(t);

                for (var cid in targetPos) {
                    var from = fromPos[cid] || { y: 0, barW: 0, avg: 0 };
                    positions[cid].y = from.y + (targetPos[cid].y - from.y) * et;
                    positions[cid].barW = from.barW + (targetPos[cid].barW - from.barW) * et;
                    positions[cid].avg = from.avg + (targetPos[cid].avg - from.avg) * et;
                }

                drawFrame();

                if (t < 1) {
                    animFrame = requestAnimationFrame(step);
                } else {
                    currentDay = targetDay;
                    transitioning = false;
                    updateProgress();
                    if (callback) callback();
                }
            }
            animFrame = requestAnimationFrame(step);
        }

        function playNext() {
            if (!playing) return;
            if (currentDay >= dates.length - 1) {
                playing = false;
                setPlayIcon('fas fa-play');
                return;
            }
            transitionToDay(currentDay + 1, playNext);
        }

        playBtn.addEventListener('click', function() {
            if (playing) {
                playing = false;
                if (animFrame) cancelAnimationFrame(animFrame);
                transitioning = false;
                setPlayIcon('fas fa-play');
            } else {
                if (currentDay >= dates.length - 1) {
                    currentDay = 0;
                    initPositions(0);
                    updateProgress();
                    drawFrame();
                }
                playing = true;
                setPlayIcon('fas fa-pause');
                playNext();
            }
        });

        // Store cleanup function for when Dashboard re-renders
        container._raceCleanup = function() {
            playing = false;
            if (animFrame) cancelAnimationFrame(animFrame);
        };

        // Init
        initPositions(0);
        updateProgress();
        drawFrame();
    },
```

- [ ] **Step 2: Commit**

```bash
git add study-tools/dashboard/rankings-viz.js
git commit -m "feat: implement bar chart race with animation and controls"
```

---

### Task 6: Bar Chart Race CSS

**Files:**
- Modify: `study-tools/dashboard/dashboard.css`

- [ ] **Step 1: Add bar race styles at the end of the file**

Append to the end of `dashboard.css`:

```css
/* Bar Chart Race */
.rankings-bar-race {
    margin-top: 24px;
}

.race-date-display {
    text-align: center;
    font-size: 1.3rem;
    font-weight: 700;
    color: var(--text, #333);
    margin-bottom: 12px;
}

.race-controls {
    display: flex;
    align-items: center;
    gap: 16px;
    margin-top: 12px;
    flex-wrap: wrap;
}

.race-play-btn {
    width: 40px;
    height: 40px;
    border-radius: 50%;
    background: var(--primary, #4e79a7);
    color: #fff;
    border: none;
    cursor: pointer;
    font-size: 1rem;
    display: flex;
    align-items: center;
    justify-content: center;
    transition: background 0.2s;
}

.race-play-btn:hover {
    opacity: 0.9;
}

.race-play-btn:disabled {
    opacity: 0.4;
    cursor: not-allowed;
}

.race-speed-wrap {
    display: flex;
    align-items: center;
    gap: 6px;
    font-size: 0.85rem;
    color: var(--text-muted, #888);
}

.race-speed-slider {
    width: 80px;
    cursor: pointer;
}

.race-speed-readout {
    font-weight: 600;
    min-width: 2em;
    color: var(--text, #333);
}

.race-progress {
    font-size: 0.85rem;
    color: var(--text-muted, #888);
}

.rankings-bar-race canvas {
    display: block;
    width: 100%;
}
```

- [ ] **Step 2: Commit**

```bash
git add study-tools/dashboard/dashboard.css
git commit -m "feat: add bar chart race styles to dashboard"
```

---

### Task 7: Wire Up Dashboard — Data Fetch and Render Call

**Files:**
- Modify: `study-tools/dashboard/dashboard.js`

- [ ] **Step 1: Add viz container reference at top of `loadOverview()`**

Find the line where `classLbContainer` is defined (search for `var classLbContainer = document.getElementById('overview-class-lb');`). After it, add:

```js
        var vizContainer = document.getElementById('overview-rankings-viz');
```

Then find the clearing block (where `if (classLbContainer) classLbContainer.textContent = '';` is). After it, add:

```js
        if (vizContainer) {
            if (vizContainer._raceCleanup) vizContainer._raceCleanup();
            vizContainer.textContent = '';
        }
```

- [ ] **Step 2: Add rankings viz render call**

Find the section where `overview-replay` is rendered. Search for `// Render leaderboard replay`:

```js
            // Render leaderboard replay
            var replayContainer = document.getElementById('overview-replay');
```

Insert the following BEFORE it:

```js
            // Render class rankings visualization
            if (vizContainer && typeof RankingsViz !== 'undefined') {
                if (vizContainer._raceCleanup) vizContainer._raceCleanup();

                var vizQuery = this.supabase.from('leaderboard_snapshots')
                    .select('student_id, score, class_id, snapshot_date')
                    .order('snapshot_date', { ascending: true })
                    .limit(5000);
                if (filters.unitId) vizQuery = vizQuery.eq('unit_id', filters.unitId);
                var vizRes = await vizQuery;
                var vizData = vizRes.data || [];

                if (filters.classId) {
                    vizData = vizData.filter(function(r) { return r.class_id === filters.classId; });
                }

                var vizClassMap = {};
                (classesRes.data || []).forEach(function(c) { vizClassMap[c.id] = c.name || c.code; });

                RankingsViz.render(vizContainer, vizData, vizClassMap);
            }

```

- [ ] **Step 3: Commit**

```bash
git add study-tools/dashboard/dashboard.js
git commit -m "feat: wire up rankings viz data fetch and render in dashboard"
```

---

### Task 8: Investigate and Fix Replay Button Bug

**Files:**
- Modify: `study-tools/dashboard/dashboard.js` (if bug is confirmed)

- [ ] **Step 1: Investigate the replay prev/next button behavior**

Read the `_renderReplay` method carefully (lines 851-1114). The buttons are wired at lines 1105-1110. Check:
1. Are there JS errors in the console when clicking?
2. Is `availableDates.length` only 1 (making both buttons disabled)?
3. Is the `loadDay` function being called but failing silently?

If the issue is that both buttons are disabled because there's only 1 day of snapshot data, this is expected behavior, not a bug — document this finding. If it's a real bug, fix it.

- [ ] **Step 2: Commit if a fix was made**

```bash
git add study-tools/dashboard/dashboard.js
git commit -m "fix: resolve replay panel button navigation issue"
```

---

### Task 9: Version Bump

**Files:**
- Modify: `study-tools/engine/version.json`

- [ ] **Step 1: Bump version**

Update `study-tools/engine/version.json`:

```json
{
    "version": "7.12.0",
    "date": "2026-03-19"
}
```

- [ ] **Step 2: Commit**

```bash
git add study-tools/engine/version.json
git commit -m "chore: bump version to 7.12.0"
```

- [ ] **Step 3: Manual smoke test**

Open the dashboard in a browser and verify:
1. Overview tab shows "Class Rankings Over Time" section with a line chart
2. Line chart has one line per class with distinct colors
3. Hovering over data points shows a tooltip with class name, date, and average
4. Below the chart, horizontal bars show class rankings
5. Play button starts the animation, bars slide and reorder smoothly
6. Pause button stops animation mid-transition
7. Speed slider changes animation speed (readout shows "1x", "2x", etc.)
8. Progress shows "Day N of M"
9. Animation auto-pauses at the last day
10. Pressing play after reaching the end restarts from day 1
11. Legend below the line chart shows class names with color swatches
