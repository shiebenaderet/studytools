StudyEngine.registerActivity({
    ...
    // Render the details panel for a clicked key term. Looks the term up in
    // config.vocabulary (case-insensitive), shows definition + simple
    ...
    openQuestion(index) {
        ...
        if (q.keyTerms && q.keyTerms.length > 0) {
            ...
            // Details panel for the expanded term (initially hidden, populated on click)
            var detailsPanel = document.createElement('div');
            detailsPanel.className = 'key-term-details';
            ...
            q.keyTerms.forEach(function(term) {
                ...
                chip.type = 'button';
                chip.className = 'key-term-chip key-term-chip-clickable';
                chip.textContent = term;
                chip.addEventListener('click', function() {
                    // Toggle off if clicking the active chip
                    if (activeChip === chip) {
                        detailsPanel.style.display = 'none';
                        chip.classList.remove('key-term-chip-active');
                        activeChip = null;
                        return;
                    }
                    if (activeChip) activeChip.classList.remove('key-term-chip-active');
                    chip.classList.add('key-term-chip-active');
                    activeChip = chip;
                    self._renderTermDetails(detailsPanel, term);
                });
                termsList.appendChild(chip);
            });
            termsDiv.appendChild(termsList);
            termsDiv.appendChild(detailsPanel);
